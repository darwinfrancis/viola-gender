...
internal abstract class Classifier protected constructor(
    ...
    init {
        ...
        Util.printLog("Created a Tensor flow Lite Gender Classifier.")
    }
    ...
    fun recognizeImage(bitmap: Bitmap, sensorOrientation: Int): List<GenderRecognition> {
    ...
    companion object {
        ...
        fun create(
            ...
            return if (model == Model.QUANTIZED_MOBILE_NET) {
                GenderClassifierQuantizedEfficientNet(context, device, numThreads)
            } else {
                throw UnsupportedOperationException()
            }
        }

        private fun getTopKProbability(labelProb: Map<String, Float>): List<GenderRecognition> {
            // Find the best classifications.
            val pq = PriorityQueue<GenderRecognition>(
                MAX_RESULTS
            ) { lhs, rhs -> // Intentionally reversed to put high confidence at the head of the queue.
                (rhs.confidence).compareTo(lhs.confidence)
            }
            for ((key, value) in labelProb) {
                pq.add(GenderRecognition(key, value))
            }
            val recognitions = ArrayList<GenderRecognition>()
